    ...
    <Card>
      ...
      <form onSubmit={handleEdit}>
        <div className="flex items-center gap-4 mb-8">
          <div className="w-20 h-20 rounded-3xl bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-2xl overflow-hidden">
            ...
            {(imgAvatar && (
              <img
                src={
                  imgAvatar instanceof File
                    ? URL.createObjectURL(imgAvatar)
                    : imgAvatar.startsWith("http")
                      ? imgAvatar
                      : `http://localhost:5000${imgAvatar}`
                }
                alt="avatar"
        ...
        <button
          type="submit"
          onClick={() => playSound()}
          className="mt-8 px-6 py-3 rounded-xl bg-indigo-600 text-white font-medium hover:bg-indigo-700"
        >
          Save Changes
        </button>
      </form>
