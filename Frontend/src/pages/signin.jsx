  ...
  return (
    <div className="login-container">
      <div className="login-content">
        <div id="logo-content flex flex-col justify-center items-center">
          ...
          <div id="logo-discp flex">
            ...
            <p className="text-sm text-gray-500">
              Sign in to continue to AirWrite
            </p>
          </div>
        </div>
        ...
        <form action={formAction} method="post">
          <div className="form-content">
            <div className="email-field relative">
              ...
              <input
                ...
                required
                onFocus={() => setRemember(true)}
              />
              <FiMail className="icons position-absolute top-10" />
              {data?.error?.email && (
                <p className="form-error">{data?.error?.email}</p>
              )}
              {remember && (
                <div
                  className="absolute w-60 h-8 p-1 rounded-b-md top-16 right-0 z-15 shadow-md bg-gray-100 text-xs font-stretch-semi-condensed text-gray-700  flex items-center justify-center"
                  onClick={() => {
                    document.getElementById("email").value = email;
                    setRemember(!remember);
                  }}
                >
                  {email ? "email: " + email : "No email saved"}
                </div>
              )}
              <br />
            </div>
            ...
            <div className="password-field">
              ...
              {data?.error?.password && (
                <p className="form-error">{data?.error?.password}</p>
              )}
            </div>
          </div>
          ...
          <div className="btn-other">
            <button
              type="button"
              className="continue-btn social-btn cursor-pointer"
              onClick={(e) => {
                e.preventDefault();
            ...
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                window.location.href = "http://localhost:5000/auth/github","_self";
              }}
            >
      ...
      >
        ...
        <path
          d="M 0,400 L 0,225 C 125.19999999999999,276.2 250.39999999999998,327.4 408,292 C 565.6,256.6 755.5999999999999,134.6 933,84 C 1110.4,33.400000000000006 1275.2,54.2 1440,75 L 1440,400 L 0,400 Z"
          stroke="none"
          stroke-width="0"
          fill="url(#gradient)"
          fill-opacity="1"
